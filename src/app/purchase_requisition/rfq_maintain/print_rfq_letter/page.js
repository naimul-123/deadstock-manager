...
const Project = ({ params }) => {
    ...
    return (projection ?
        <div className="min-h-[calc(100vh-44px)] grow">
            <div className="flex p-2  gap-2 justify-between items-center bg-[#D9EEE1] print:hidden  ">
            ...
            <div className="font-[sutonnyOMJ] mx-auto p-16 break-after-page ">
            ...
            {
                projection.rfqInfo?.vendors?.map((vendor, idx) => <div key={vendor.vendor_id} className={`bg-base-300 font-[sutonnyOMJ] mx-auto p-16 hidden print:block ${idx === projection.rfqInfo?.vendors?.length - 1 ? "" : "break-after-page"} `} >
                    <div className={`flex justify-between items-end`}>
                        <div className={`w-20 h-20 ${!isOpenHeadline ? "invisible" : "visible"}`}><Image alt="logo" src={logo} /></div>
                        <div className={`font-bold text-center ${!isOpenHeadline ? "invisible" : "visible"}`} >
                            <h2 className="text-3xl">বাংলাদেশ ব্যাংক</h2>
